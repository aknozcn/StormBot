        ...
        private void OnClientConnect(IAsyncResult ar)
        {
            if (!isClosing)
            {
                try
                {
                    ...
                    local_socket.BeginReceive(local_recv_buffer.Buffer, 0, MAX_RECV_SIZE, SocketFlags.None, new AsyncCallback(WaitForData), local_socket);
        ...
        private void WaitForData(IAsyncResult ar)
        {
            if (!isClosing && doPacketProcess)
            {
                ...
                finally
                {
                    if (worker != null)
                    {
                        worker.BeginReceive(local_recv_buffer.Buffer, 0, MAX_RECV_SIZE, SocketFlags.None, new AsyncCallback(WaitForData), worker);
                    }
                }
            }
        }
        ...
        private void _processClientPackets()
        {
            if (!isClosing && doPacketProcess)
            {
                ...
                if (local_send_buffers != null)
                {
                    ...
                    {
                        Console.WriteLine("P->C:" + buffer.Value.Opcode.ToString("X4") + " Data:" + buffer.Value.ToString());
